...
namespace libhand {
        ...
        public:
            ...
            void walk_bones(HandRenderer::JointPositionMap &jointPositionMap, Node* bone = NULL, int depth = 0) {
                ...
                if (bone->numChildren() == 0) {
                    ...
                    Vector3 tip_pos = pos_world + 0.15 * step;
    ...
    void HandRenderer::GetMeshVertices(size_t &vertex_count,
            Vector3* &vertices) {
        private_->GetMeshVertices(vertex_count, vertices);
    }
    ...
    void HandRendererPrivate::Setup(int width, int height) {
        ...
        if(!root_)
        {
            ...
            root_->loadPlugin("/usr/lib/OGRE/RenderSystem_GL.so");
            root_->loadPlugin("/usr/lib/OGRE/Plugin_OctreeSceneManager.so");
    ...
    void HandRendererPrivate::GetMeshVertices(size_t &vertex_count,
            Vector3* &vertices) {
        ...
        MeshPtr mesh = hand_entity_->getMesh();

        // Calculate the number of vertices we need for memory allocation
        ...
        for (unsigned short i = 0; i < mesh->getNumSubMeshes(); i++) {
            ...
            if ((!submesh->useSharedVertices) || (submesh->useSharedVertices && !added_shared)) {
                ...
                const VertexElement* pos_elem = vertex_data->vertexDeclaration->findElementBySemantic(VES_POSITION);

                HardwareVertexBufferSharedPtr vbuf = vertex_data->vertexBufferBinding->getBuffer(pos_elem->getSource());
                ...
                for (size_t j = 0; j < vertex_data->vertexCount; j++, vertex += vbuf->getVertexSize()) {
                    ...
                    Vector3 pt(p_real[0], p_real[1], p_real[2]);
